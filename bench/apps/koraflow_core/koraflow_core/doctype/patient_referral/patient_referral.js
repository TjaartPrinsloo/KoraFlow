...
frappe.ui.form.on('Patient Referral', {
			...
			if (frappe.user.has_role('Sales Agent')) {
				frm.add_custom_button(__('Message Sales Team'), function () {
					frappe.prompt([
						...
						frappe.call({
							method: 'koraflow_core.doctype.referral_message.referral_message.create_message',
							args: {
	...
	patient: function (frm) {
		...
		if (frm.doc.patient) {
			frappe.call({
				method: 'koraflow_core.doctype.patient_referral.patient_referral.fetch_patient_names',
				args: {
	...
	sales_agent: function (frm) {
		...
		if (frm.doc.sales_agent && !frm.doc.sales_partner) {
			frappe.call({
				method: 'koraflow_core.doctype.patient_referral.patient_referral.get_sales_partner_for_agent',
				args: {
