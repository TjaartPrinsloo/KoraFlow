...
def on_user_insert(doc, method):
	...
	if not frappe.db.exists("Notification Settings", doc.name):
		try:
			...
			frappe.log_error(title="User Creation", message=f"Error creating notification settings for {doc.name}: {str(e)}")
	...
	if doc.user_type == "Patient":
		if not frappe.db.get_value("Patient", {"email": doc.email}, "name"):
			try:
				...
				frappe.log_error(title="User Insert Error", message=f"Error in on_user_insert for {doc.email}: {str(e)}")
