		...
		if (frm.doc.name && !frm.is_new()) {
			...
			function addPrescriptionLink(count, prescriptions) {
				...
				function tryAdd() {
					...
					// Try to find the form links area
					const formLinksArea = $('.form-documents');
					
					if (!formLinksArea.length && attempts < maxAttempts) {
						setTimeout(tryAdd, 200);
						return;
					}
					
					if (!formLinksArea.length) {
						console.warn('Could not find form-documents area');
						return;
					}
					...
					// Alternative: find by title text
					...
					if (appointmentsSection && appointmentsSection.length) {
						...
						if (!appointmentsSection.find('.document-link[data-prescription-link="true"]').length) {
							...
							console.log('Prescriptions link added successfully');
						} else {
						...
						console.warn('Could not find Appointments and Encounters section');
					}
				}
