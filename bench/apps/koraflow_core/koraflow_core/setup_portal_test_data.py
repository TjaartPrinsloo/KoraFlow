...
def setup_test_data():
    ...
    # 1. Create or Get Test User & Patient
    ...
    # Create Patient linked to User and Customer
    ...
    if not patient_name:
        ...
        patient.customer = customer_name # Link customer
    ...
    # Check if DocType exists first to avoid error if I hallucinated the name
    ...
    # 5. Create Quotation
    quotes = frappe.get_all("Quotation", filters={"party_name": customer_name, "status": "Open"})
    if not quotes:
        ...
        quote.append("items", {
            "item_code": "GLP1-025", # Assuming item exists or simple text
            "item_name": "GLP-1 Starter Kit",
            "qty": 1,
            "rate": 1500
        })
        # If item doesn't exist, this might fail. Let's ensure item.
