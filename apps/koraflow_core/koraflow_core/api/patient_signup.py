...
def send_verification_email(user, verification_url):
	"""Send branded email verification email to user.
	Uses 'Slim2Well - Email Verification' Email Template if available.
	...
	template_name = "Slim2Well - Email Verification"
	if frappe.db.exists("Email Template", template_name):
		email_template = frappe.get_doc("Email Template", template_name)
		subject = frappe.render_template(email_template.subject, context)
		message = frappe.render_template(email_template.response, context)
	else:
		subject = _("Verify Your Email — Slim2Well")
		message = f"""<p>Hi {user.first_name},</p>
		<p>Thank you for signing up with Slim2Well! Please verify your email:</p>
...
def send_welcome_email(user):
	...
	Uses 'Slim2Well - Welcome' Email Template if available.
	"""
	context = {
		"first_name": user.first_name or "there",
	}

	template_name = "Slim2Well - Welcome"
	try:
		if frappe.db.exists("Email Template", template_name):
			email_template = frappe.get_doc("Email Template", template_name)
			subject = frappe.render_template(email_template.subject, context)
			...
			subject = "Welcome to Slim2Well!"
			message = f"""<p>Hi {user.first_name},</p>
			<p>Welcome to Slim2Well! Your account is ready. Please complete your health intake form to get started.</p>
			<p>Kind regards,<br>The Slim2Well Team</p>"""
		...
		frappe.log_error(title="Welcome Email Error", message=str(e))
