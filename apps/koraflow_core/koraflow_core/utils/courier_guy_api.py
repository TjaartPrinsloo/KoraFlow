...
class CourierGuyAPI:
	...
	def _make_request(self, method: str, endpoint: str, data: Optional[Dict] = None) -> Dict:
		...
		try:
			...
			# Capture the response body for debugging — ShipLogic returns
			# error details in the body even on 500 errors.
			error_msg = str(e)
			response_body = ""
			status_code = None
			...
			full_error = f"{error_msg}\nResponse body: {response_body}" if response_body else error_msg
			log_title = f"Courier Guy API Error: {error_msg[:100]}"
			frappe.log_error(title=log_title, message=full_error)
			return {
				"success": False,
				"error": full_error,
	...
	def create_waybill(self, waybill_doc) -> Dict:
		...
		# Clean street addresses — API doesn't accept newlines
		pickup_street = (waybill_doc.pickup_address or "").replace("\n", ", ").strip()
		delivery_street = (waybill_doc.delivery_address or "").replace("\n", ", ").strip()

		# Resolve delivery zone (province) from address doc if available
		delivery_zone = ""
		if waybill_doc.delivery_note:
			dn = frappe.get_doc("Delivery Note", waybill_doc.delivery_note)
			if dn.shipping_address_name:
				delivery_zone = frappe.db.get_value("Address", dn.shipping_address_name, "state") or ""

		shipment_data = {
			"collection_address": {
				"type": "business",
				"company": waybill_doc.pickup_name or "",
				"street_address": pickup_street,
